...
namespace AppMasters.Extensions.UI
{
    /// <summary>
    /// Extension featuring <br>
    /// onValueChanged event ()
    /// public Toggles list (line 29)
    /// </summary>Z
    public class ToggleGroup : UIBehaviour
    {
        [SerializeField] private bool m_AllowSwitchOff = false;
        [SerializeField] private bool m_AllowOnlyOne = false;
        ...
        public UnityEvent<Toggle> m_OnToggleOn = new UnityEvent<Toggle>();
        ...
        public void UnregisterToggle(Toggle toggle)
        {
            if (m_Toggles.Contains(toggle))
            {
                ...
                toggle.onValueChanged.RemoveListener((isOn) => ToggleChanged(toggle, isOn));
            }
        }
        ...
        void ToggleChanged(Toggle toggle, bool active)
        {
            m_OnValueChanged?.Invoke();
            
            if (!active) return;

            m_OnToggleOn?.Invoke(toggle);

            var activeToggles = ActiveToggles();

            if (active && m_AllowOnlyOne && activeToggles.Count() > 1)
            {
        ...
        private void LateUpdate() {
            // Debug.Log($"{ActiveToggles().Count()} {ActiveToggles().FirstOrDefault()}");
        }
